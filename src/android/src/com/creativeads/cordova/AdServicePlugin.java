package com.creativeads.cordova;
...
public class AdServicePlugin extends CordovaPlugin implements
    ...
    public void onPause(boolean multitasking) {
        Log.d(TAG, "onPause...");
        handlePause(this.cordova.getActivity());
    }

    @Override
    public void onResume(boolean multitasking) {
        Log.d(TAG, "onResume...");
        handleResume(this.cordova.getActivity());
    }
    ...
    private void handlePause(Context ctx) {
        ...
        for (String key : _rewardedVideos.keySet()) {
            AdRewardedVideo rewardedVideo = _rewardedVideos.get(key);
            Log.d(TAG, "pausing..." + key);

            rewardedVideo.onPause(ctx);
        }
    }
    ...
    private void handleResume(Context ctx) {
        ...
        for (String key : _rewardedVideos.keySet()) {
            AdRewardedVideo rewardedVideo = _rewardedVideos.get(key);
            Log.d(TAG, "resuming..." + key);

            rewardedVideo.onResume(ctx);
        }
    }
